...
/* Compose and managed deployments set CLOSURE_API_URL explicitly; the
 * localhost default covers ad-hoc host runs. */
const SPEC_URL =
  process.env.CLOSURE_API_URL ??
  `http://localhost:${process.env.CLOSURE_API_PORT ?? "3216"}/openapi.json`;
...
const writeSpecIfChanged = async () => {
  ...
  if (!response.ok) {
    throw new Error(`GET ${SPEC_URL} -> ${response.status}`);
  }
 ...
 * outlast a not-yet-serving API before giving up: the patience budget is
 * BOOT_ATTEMPT_LIMIT x BOOT_RETRY_DELAY_MS of wall-clock time. */
const writeSpecWithRetries = async () => {
  for (let attempt = 1; attempt <= BOOT_ATTEMPT_LIMIT; attempt++) {
    try {
      ...
      await sleep({ ms: BOOT_RETRY_DELAY_MS });
    }
  }
