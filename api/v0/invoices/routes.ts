/**
 * v0/invoices/routes.ts -- HTTP for /v0/tenants/:id/invoices: request
...
import { taxRefSchema, tenantParam, valueRefSchema } from "../helpers.ts";
import {
  closeInvoice,
  createInvoice,
  getInvoice,
...
export const invoicesApp = new Hono()
  .post("/", zValidator("json", invoiceCreateSchema), async (c) => {
    const body = c.req.valid("json");
    return c.json(
      await createInvoice({ invoice: body, tenantId: tenantParam(c) }),
      201,
    );
  })
  .get("/", async (c) => {
    return c.json(await listInvoices({ tenantId: tenantParam(c) }));
  })
  .get("/:iid", async (c) => {
    const invoice = await getInvoice({ uniqueId: c.req.param("iid") });
    if (!invoice) {
      return c.json({ error: "not found" }, 404);
    }
    return c.json(invoice);
  })
  .post("/:iid/close", zValidator("json", closeInvoiceSchema), async (c) => {
    const invoice = await closeInvoice({
      body: c.req.valid("json"),
      uniqueId: c.req.param("iid"),
    });
    if (!invoice) {
      return c.json({ error: "not found" }, 404);
    }
