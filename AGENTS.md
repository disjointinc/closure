...
Keep your answers as concise as possible. Use as simple, non-jargony language as possible. Bulletpoints > paragraphs.
...
Ensure all resources created by tests are cleaned up fully. Ideally, that's by using API routes, but if not, that's by using garbage collection. Garbage collection scripts should be invoked directly by tests rather than relying on loops; we don't want to risk deleting resources in production by adding any test garbage colllection loops to the production garbage collection loop.
